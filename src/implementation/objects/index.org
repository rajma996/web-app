...
#+BEGIN_SRC python :tangle ../../tests/test_obj.py :eval no
    def test_adduser(self):
        temp = {'name': Name('john'), 'email': Email('johnsnow@gmail.com') , 'role': Role('admin') }
        temp1 = {'name': Name('johnsnow'), 'email': Email('johnsnow@gmail.com') , 'role': Role('admin') }
        inst = User(temp)
        inst1 = User(temp1)
        inst.add_user();
...
class TestSession(TestCase):
    ...
    def test_session_creation(self):
        ...
        ses.add_session()
        ses1.add_session()
        if ses not in Session.session_list or ses1 not in Session.session_list :
            raise ConstraintError("Multiple sessions are allowed ")

        if ses.user != ses1.user :
            raise ConstraintError("Multiple sessions are allowed ")
#+END_SRC
...
#+BEGIN_SRC python :tangle ../../src/obj.py :eval no
    def add_session(self):
        if self not  in Session.session_list :
            Session.session_list.append(self)
        else :
            raise ConstraintError('Session already exists')
...
#+BEGIN_SRC python :tangle ../../tests/test_obj.py :eval no
    def test_addsession(self):
        ...
        temp = {'name': Name('john'), 'email': Email('johnsnowa@gmail.com') , 'role': Role('admin') }
        inst = User(temp)
        inst.add_user()
        s = Session(inst)
        s.add_session()
...
#+BEGIN_SRC python :tangle ../../src/obj.py :eval no
    def add_user(self,user):
        if self.user.get_role()!='admin' and self in Session.session_list:
            raise ConstraintError ('Only an admin can add an user')
        elif not isinstance(user,User):
            raise AttributeError ('ONly instance of User class can be made a user')
        else :
            user.add_user()
#+END_SRC
...
#+BEGIN_SRC python :tangle ../../tests/test_obj.py :eval no
    def test_adduser(self):
        print 'testing Session:add_user'
        temp = {'name': Name('john'), 'email': Email('john@gmail.com') , 'role': Role('admin') }
        inst = User(temp)
        inst.add_user()
        ses = Session(inst)
        ses.add_session()
        temp = {'name': Name('johnsnow'), 'email': Email('johnsnow@gmail.com') , 'role': Role('normal') }
        inst1 = User(temp)
        ses.add_user(inst1)
        if inst1 not in User.users:
            raise ConstraintError('User not added')
#+END_SRC
